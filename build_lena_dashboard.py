...
CUISINE_RU = {
    ...
    "Asian": "Азиатская", "PanAsian": "Паназиатская", "French": "Французская", "Seafood": "Морепродукты",
...
def _mock_leads():
    return [
        ...
        },
        {
            ...
            "Кухня": [{"value": "PanAsian"}],
            "Средний чек, ₽": 1500,
            "Ссылка на 2gis или Яндекс": "https://yandex.ru/maps/?ll=37.6435,55.7616&z=17&pt=37.6435,55.7616",
            "Есть ли свой сайт?": {"value": "no"},
            "Telegram": "https://t.me/jorikubilhamleta",
            "Телефон ресторана": "+7 (495) 129-06-69",
            "Рейтинг 2gis/Яндекс": 47,
            "Сколько оценок": 2074,
            "Что особенного (в 2 предложениях)": (
                "Шекспировский бар на Покровке — концепция «истинная цена» меняет цены на алкоголь до 19:00. "
                "30+ сортов джина и джин-конструктор с именами персонажей Шекспира, "
                "воскресная ценовая рулетка с бонусами и регулярные DJ-сеты. "
                "Паназиатское меню: дим-сам, лапша, кимчи — и хороший джин."
            ),
            "Уникальная концепция?": {"value": "yes strong"},
            "Банкеты": {"value": "yes"},
            "Афиша событий": {"value": "yes regular"},
...
def generate_pitches(lead):
    ...
    vk_target = vk or name
    vk_channel_hint = (
        "Нашла в Яндекс.Картах и Telegram-канале" if (not vk and tg_pitch)
        else "Заметила что нет своего сайта — только ВКонтакте и Яндекс.Карты"
    )
    vk_prompt = (
        f"{ctx}\n\n"
        f"Напиши короткое сообщение во ВКонтакте или по email на заведение {vk_target}. "
        f"До 4 предложений. {vk_channel_hint}. "
    ...
    phone_prompt = (
        ...
        "Предложи демо-сайт. Обязательно упомяни конкретные фишки из «Что особенного» — "
        "не просто «бар», а именно те детали которые выделяют заведение. "
        "Формат: блоки 'Лена:' и 'Если говорят...' (2-3 возражения)."
    )
    email_prompt = (
        f"{ctx}\n\n"
        ...
        "Упомяни конкретные фишки из «Что особенного» — покажи что изучила заведение. "
...
def build_card(lead):
    ...
    tg_short = tg.split("t.me/")[-1] if "t.me/" in tg else tg
    ...
    if tg:
        contact_rows.append(info_row("✈️", "Telegram-канал",
            f'<a href="{h(tg)}" target="_blank" class="info-link">t.me/{h(tg_short)}</a>'))
    if yandex:
